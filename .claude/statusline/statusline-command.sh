...
# Format token counts as human-readable (e.g. 45k/200k)
format_k() {
  val="$1"
  if [ -z "$val" ]; then echo "?"; return; fi
  awk "BEGIN { if ($val >= 1000) printf \"%.0fk\", $val/1000; else printf \"%d\", $val }"
}

if [ -n "$used" ] && [ -n "$ctx_total" ]; then
  ctx_used=$(awk "BEGIN { printf \"%.0f\", $used * $ctx_total / 100 }")
  token_str="$(format_k "$ctx_used")/$(format_k "$ctx_total") tokens"
else
  token_str=""
fi
...
if [ -n "$token_str" ]; then
  printf "⏱️ %s | 🤖 %s\n🧠 %s (%s) | 💰 %s | 📁 %s\n🌳 %s | 🌿 %s" "$rate_limit_str" "$model" "$usage_str" "$token_str" "$block_str" "$dir_display" "$worktree_str" "$git_str"
else
  printf "⏱️ %s | 🤖 %s\n🧠 %s | 💰 %s | 📁 %s\n🌳 %s | 🌿 %s" "$rate_limit_str" "$model" "$usage_str" "$block_str" "$dir_display" "$worktree_str" "$git_str"
fi
