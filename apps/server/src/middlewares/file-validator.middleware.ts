...
export class FileValidatorMiddleware {
  public handle = (options: FileValidatorOptions): RequestHandler => {
    ...
    return (req, res, next) => {
      multerHandler(req, res, (err: unknown) => {
        ...
        const files = this.normaliseFiles(rawFiles);
        const validationError = this.validateFiles(files, options);
